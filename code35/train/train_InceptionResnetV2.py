...
def train(mission_id, BATCH_SIZE, EPOCH, LR, LR_DECAY, DECAY_EPOCH=(30, 50, 70)):
    ...
    log_txt = mission_kind + ' InceptionResnetV2 {} BZ_{} EPO_{} LR_{} LR_DE_{} DE_EPO_{}'. \
    ...
    model.last_linear = nn.Linear(1536, IMAGE_CLASS)
    #print(model)
    ...
    for epoch in range(1, EPOCH + 1):
        ...
        for step, (x, y) in enumerate(train_loader):
            ...
            running_corrects += torch.sum(preds.cpu().data== y.cpu().data)
            #print(step, running_corrects, type('\n'))
        ...
        info += '[{:.1f}mins/{}pics]'.format(use_time / 60.0, train_speed)
        print (info)
        log_txt.write(info.encode() + '\n'.encode())
        if TOP_ACC < epoch_acc:
            TOP_ACC = epoch_acc
            model_sava_path = os.path.join(model_dir, mission_kind + '_InceptionResnetV2_%s_%.4f.pkl' % (epoch, epoch_acc))
            torch.save(model, model_sava_path)
            torch.save(model, os.path.join(model_dir, 'InceptionResnetV2_best_model.pkl'))
            acc_info = 'save epoch:{} acc:{}'.format(epoch, epoch_acc)
            print (acc_info)
            log_txt.write(acc_info.encode() + '\n'.encode())
    log_txt.close()


if __name__ == '__main__':
    fire.Fire(train)
    #train(3, BATCH_SIZE=16, EPOCH=100, LR=0.01, LR_DECAY=0.1, DECAY_EPOCH=(30, 50, 70))
